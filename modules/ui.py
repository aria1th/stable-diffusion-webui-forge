...
def create_sampler_and_steps_selection(choices, tabname):
    if opts.samplers_in_dropdown:
        with FormRow(elem_id=f"sampler_selection_{tabname}"):
            ...
            steps = gr.Slider(minimum=1, maximum=28, step=1, elem_id=f"{tabname}_steps", label="Sampling steps", value=20)
    else:
...
def create_ui():
    ...
    with gr.Blocks(analytics_enabled=False,visible=False) as img2img_interface:
    ...
    with gr.Blocks(analytics_enabled=False,visible=False) as train_interface:
        ...
        with ResizeHandleRow(variant="compact", equal_height=False, visible=False):
    ...
    with gr.Blocks(theme=shared.gradio_theme, analytics_enabled=False, title="Stable Diffusion") as demo:
        ...
        with gr.Tabs(elem_id="tabs") as tabs:
            ...
            for interface, label, ifid in sorted_interfaces:
                if label in shared.opts.hidden_tabs or label in ["img2img", "Extras", "Checkpoint Merger", "Train", "Extensions"]:
                    continue
